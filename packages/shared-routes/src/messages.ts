...
import type { ElementId, EntityId, Channel, Message, Element, CreateMessageInput } from '@slice/core';
import { createDocument, createMessage, DocumentCategory } from '@slice/core';
...
export function createMessageRoutes(services: CollaborateServicesWithBroadcast) {
    ...
    try {
          ...
          if (memberId !== senderId && !notifiedEntities.has(memberId)) {
            try {
              ...
              if ((error as { code?: string }).code !== 'ALREADY_EXISTS') {
                console.error(`[slice] Failed to create inbox item for ${memberId}:`, error);
              }
            }
          }
        }
      ...
      for (const mention of mentions) {
        ...
        try {
          ...
          if ((error as { code?: string }).code !== 'ALREADY_EXISTS') {
            console.error(`[slice] Failed to process mention ${mentionedId}:`, error);
          }
        }
      }
      ...
      if (body.threadId) {
        try {
          ...
          // Ignore errors
          if ((error as { code?: string }).code !== 'ALREADY_EXISTS') {
            console.error(`[slice] Failed to create thread reply inbox item:`, error);
          }
        }
      }
      ...
      console.error('[slice] Failed to create message:', error);
  ...
  app.get('/api/messages/search', async (c) => {
    try {
      ...
      console.error('[slice] Failed to search messages:', error);
  ...
  app.get('/api/messages/:id/replies', async (c) => {
    try {
      ...
      console.error('[slice] Failed to get thread replies:', error);
