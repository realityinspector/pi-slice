    ...
    try {
      ...
      for (const event of events) {
        const wsEvent: WebSocketEvent = {
          id: event.id,
          elementId: event.element_id as import('@slice/core').ElementId,
          eventType: event.event_type as import('@slice/core').EventType,
          actor: event.actor as import('@slice/core').EntityId,
          oldValue: event.old_value ? JSON.parse(event.old_value) : null,
          newValue: event.new_value ? JSON.parse(event.new_value) : null,
          createdAt: event.created_at as import('@slice/core').Timestamp,
