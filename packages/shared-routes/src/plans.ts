...
export function createPlanRoutes(services: CollaborateServices) {
  ...
  app.get('/api/plans', async (c) => {
    try {
      ...
      console.error('[slice] Failed to get plans:', error);
  ...
  app.get('/api/plans/:id', async (c) => {
    try {
      ...
      console.error('[slice] Failed to get plan:', error);
  ...
  app.get('/api/plans/:id/tasks', async (c) => {
    try {
      ...
      console.error('[slice] Failed to get plan tasks:', error);
  ...
  app.get('/api/plans/:id/progress', async (c) => {
    try {
      ...
      console.error('[slice] Failed to get plan progress:', error);
  ...
  app.post('/api/plans/:id/tasks', async (c) => {
    try {
      ...
      console.error('[slice] Failed to add task to plan:', error);
  ...
  app.delete('/api/plans/:id/tasks/:taskId', async (c) => {
    try {
      ...
      console.error('[slice] Failed to remove task from plan:', error);
  ...
  app.post('/api/plans', async (c) => {
    try {
      ...
      console.error('[slice] Failed to create plan:', error);
    ...
    try {
      ...
      for (const task of tasks) {
        try {
          ...
          console.warn(`[slice] Failed to remove task ${task.id} from plan ${id}:`, err);
        }
      }
      ...
      console.error('[slice] Failed to delete plan:', error);
  ...
  app.patch('/api/plans/:id', async (c) => {
    try {
      ...
      console.error('[slice] Failed to update plan:', error);
