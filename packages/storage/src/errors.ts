 ...
 *
 * Maps SQLite error codes and messages to Slice error types.
 * This provides a consistent error interface regardless of the
 * underlying SQLite backend being used.
 */

import { StorageError, ConflictError, ConstraintError, ErrorCode } from '@slice/core';
...
/**
 * Convert a SQLite error to an appropriate Slice error type
 *
 * @param error - The original SQLite error
 * @param context - Optional context about the operation that failed
 * @returns An SliceError subclass with appropriate code and details
 */
export function mapStorageError(
  ...
  // Already an Slice error, return as-is
