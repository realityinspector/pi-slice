...
import type { WebSocketEvent, SubscriptionChannel, ServerMessage } from '@slice/shared-routes';
import { parseClientMessage, shouldReceiveEvent } from '@slice/shared-routes';
...
export function broadcastInboxEvent(
  ...
  const event: WebSocketEvent = {
    ...
    elementId: itemId as import('@slice/core').ElementId,
    elementType: 'inbox-item',
    eventType: eventType as import('@slice/core').EventType,
    actor: (actor || 'system') as import('@slice/core').EntityId,
    oldValue,
    newValue: { ...newValue, recipientId }, // Include recipientId for entity-specific subscriptions
    createdAt: new Date().toISOString() as import('@slice/core').Timestamp,
  };
