...
import { createStorage, initializeSchema, type StorageBackend } from '@slice/storage';
import { createQuarryAPI } from '../api/quarry-api.js';
import type { QuarryAPI } from '../api/types.js';
import type { EntityId } from '@slice/core';
import { OPERATOR_ENTITY_ID } from './commands/init.js';
import { findSliceDir } from '../config/file.js';
...
export const DEFAULT_DB_NAME = 'slice.db';
...
export function resolveDatabasePath(options: GlobalOptions, requireExists: boolean = true): string | null {
  ...
  // Find .slice directory via STONEFORGE_ROOT env or walk-up search.
  ...
  const sliceDir = findSliceDir(process.cwd());
  if (sliceDir) {
