/**
 * serve command - Start a Slice server
 *
 * Usage:
 ...
 * where quarry cannot resolve @slice/smithy at runtime.
 */
...
function getSmithyRegistration(): SmithyRegistration | undefined {
  return (globalThis as Record<string, unknown>).__slice_smithy as SmithyRegistration | undefined;
}
...
function smithyWebRoot(): string | undefined {
  ...
  try {
    ...
    const smithyUrl = import.meta.resolve('@slice/smithy');
...
async function startQuarry(options: GlobalOptions): Promise<CommandResult> {
  ...
  console.log(`[slice] Quarry server running at http://${host}:${port}`);
...
async function startSmithy(options: GlobalOptions): Promise<CommandResult> {
  ...
  if (reg) {
    ...
    try {
      ...
      const mod = await import('@slice/smithy/server');
      startSmithyServer = mod.startSmithyServer;
    } catch {
      return failure(
        'Smithy is not installed. Install @slice/smithy to use `sf serve smithy`.',
...
export const serveCommand: Command = {
  name: 'serve',
  description: 'Start a Slice server (smithy, quarry)',
  ...
  handler: async (args: string[], options: GlobalOptions): Promise<CommandResult> => {
    ...
    try {
      ...
      try {
        // @ts-ignore — smithy is an optional runtime dependency, may not be installed
        await import('@slice/smithy/server');
        return await startSmithy(options);
      } catch {
        return await startQuarry(options);
      }
