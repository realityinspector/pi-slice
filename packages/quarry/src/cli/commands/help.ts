...
const { version: VERSION } = require('@slice/quarry/package.json');
...
function generateMainHelp(): string {
  const lines: string[] = [
    'Slice - Agent coordination system',
    '',
    'Usage: slice <command> [options]',
    '       sf <command> [options]',
    '',
    'Commands:',
  ];
...
function versionHandler(): CommandResult {
  return success({ version: VERSION }, `slice v${VERSION}`);
}
