...
import { createStorage, type StorageBackend } from '@slice/storage';
import {
  initializeSchema,
  getSchemaVersion,
  isSchemaUpToDate,
...
async function doctorHandler(
  ...
  const workspaceExists = existsSync(sliceDir);

  if (!workspaceExists && !options.db) {
    diagnostics.push({
      name: 'workspace',
      status: 'error',
      message: 'No .slice directory found',
      details: { path: sliceDir },
    });
  ...
  diagnostics.push({
    name: 'workspace',
    status: 'ok',
    message: options.db ? `Using custom database path: ${options.db}` : `Workspace found at ${sliceDir}`,
  });
  ...
  if (!dbPath) {
    diagnostics.push({
      ...
      details: { expectedPath: join(sliceDir, DEFAULT_DB_NAME) },
    });
    return buildDoctorResult(diagnostics, options);
  }
...
Database health:
- Workspace exists (.slice directory)
