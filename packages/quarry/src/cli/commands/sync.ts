...
import { resolveDatabasePath, SLICE_DIR, DEFAULT_DB_NAME } from '../db.js';
...
function resolveSyncDir(options: { output?: string; input?: string }, isExport: boolean): string {
  ...
  // Default to .slice/sync
  const sliceDir = join(process.cwd(), SLICE_DIR);
  return join(sliceDir, DEFAULT_SYNC_DIR);
}
...
const exportOptions: CommandOption[] = [
  {
    name: 'output',
    short: 'o',
    description: 'Output directory path (default: .slice/sync)',
    hasValue: true,
  },
  {
    name: 'full',
...
Options:
  -o, --output <dir>       Output directory (default: .slice/sync)
...
const importOptions: CommandOption[] = [
  {
    name: 'input',
    short: 'i',
    description: 'Input directory path (default: .slice/sync)',
    hasValue: true,
  },
  {
    name: 'dry-run',
...
Options:
  -i, --input <dir>    Input directory (default: .slice/sync)
...
async function statusHandler(
  ...
  try {
    ...
    const syncDir = join(process.cwd(), SLICE_DIR, DEFAULT_SYNC_DIR);
