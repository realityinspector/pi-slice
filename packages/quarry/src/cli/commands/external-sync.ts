...
import type { Task, Document, ElementId, ExternalProvider, ExternalSyncState, SyncDirection, SyncAdapterType, TaskSyncAdapter, DocumentSyncAdapter } from '@slice/core';
 ...
 * Uses optional peer dependency @slice/smithy.
 */
async function createSettingsServiceFromOptions(options: GlobalOptions): Promise<{
  ...
  try {
    ...
    const { createSettingsService } = await import('@slice/smithy/services');
    ...
    if (message.includes('Cannot find') || message.includes('MODULE_NOT_FOUND')) {
      return {
        settingsService: null as unknown as SettingsServiceLike,
        error: 'External sync requires @slice/smithy package. Ensure it is installed.',
      };
    }
    return {
      settingsService: null as unknown as SettingsServiceLike,
...
async function createProviderFromSettings(
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    if (message.includes('Cannot find') || message.includes('MODULE_NOT_FOUND')) {
      return { error: 'External sync requires @slice/smithy package. Ensure it is installed.' };
    }
...
When auto-link is enabled, newly created Slice elements will automatically
...
const linkCommand: Command = {
  ...
  help: `Link a Slice element (task or document) to an external issue or page.
...
Arguments:
  elementId        Slice element ID (task or document)
...
const unlinkCommand: Command = {
  ...
  usage: 'sf external-sync unlink <elementId>',
  help: `Remove the external link from a Slice task or document.
...
Optionally discover new issues not yet linked to Slice elements.
...
const biSyncCommand: Command = {
  ...
  help: `Run bidirectional sync between Slice and external services.
...
export const externalSyncCommand: Command = {
  ...
  help: `Manage bidirectional synchronization between Slice and external services
