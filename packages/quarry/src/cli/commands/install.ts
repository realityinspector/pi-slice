/**
 * Install Commands - Install slice extensions to the workspace
 *
...
/**
 * Attempts to find the skills directory from the @slice/smithy package
 */
function findSkillsSourceDir(): string | null {
  // Try multiple locations to find the skills

  // 1. Check if @slice/smithy is in node_modules (dist/skills for published package)
  const nodeModulesPath = join(process.cwd(), 'node_modules', '@slice', 'smithy');
  ...
  const globalNodeModulesPath = join(dirname(process.execPath), '..', 'lib', 'node_modules', '@slice', 'smithy', 'dist', 'skills');
...
async function installSkillsHandler(
  ...
  if (!result) {
    return failure(
      'Could not find skills to install. Make sure @slice/smithy is installed.',
...
export const installCommand: Command = {
  name: 'install',
  description: 'Install slice extensions',
  usage: 'sf install <subcommand> [options]',
  help: `Install slice extensions to the workspace.
