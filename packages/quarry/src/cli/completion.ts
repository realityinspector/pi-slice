...
export function generateBashCompletion(commands: Command[]): string {
  ...
  return `# Bash completion for slice (sf)
...
complete -F _slice_completion sf
...
${opts}
  ...
  return `#compdef sf slice
# Zsh completion for slice (sf)
...
export function generateFishCompletion(commands: Command[]): string {
  ...
  for (const opt of GLOBAL_OPTIONS) {
    ...
    lines.push(`complete -c slice ${short}-l ${opt.name} ${requiresArg}-d '${escapeFish(opt.description)}'`);
  }
  ...
  for (const cmd of commands) {
    ...
    lines.push(`complete -c slice -n '__fish_use_subcommand' -a ${cmd.name} -d '${escapeFish(cmd.description)}'`);
    ...
    if (cmd.subcommands) {
      ...
      for (const [subName, sub] of Object.entries(cmd.subcommands)) {
        ...
        lines.push(`complete -c slice -n '__fish_seen_subcommand_from ${cmd.name}' -a ${subName} -d '${escapeFish(sub.description)}'`);
      }
    }
    ...
    if (cmd.options && cmd.options.length > 0) {
      ...
      for (const opt of cmd.options) {
        ...
        lines.push(`complete -c slice -n '__fish_seen_subcommand_from ${cmd.name}' ${short}-l ${opt.name} ${requiresArg}-d '${escapeFish(opt.description)}'`);
      }
    }
  }
