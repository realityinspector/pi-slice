 ...
 * a Slice task via `externalRef` and `_externalSync` metadata.
 *
 ...
 *
 * Creates an issue on the configured provider and updates the Slice task
...
export async function autoLinkTask(params: AutoLinkTaskParams): Promise<AutoLinkTaskResult> {
  ...
  try {
    ...
    try {
      ...
      issueInput = {
        title: task.title,
        body: task.descriptionRef ? `Slice task: ${task.id}` : undefined,
