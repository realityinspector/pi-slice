 ...
 *
 * Shared field mapping logic for converting between Slice tasks and
 ...
 *
 * Key functions:
 * - taskToExternalTask: Convert a Slice Task → ExternalTaskInput for push
...
import type { ExternalTask, ExternalTaskInput } from '@slice/core';
...
export interface TaskSyncFieldMapConfig {
  ...
  /**
   * Converts a Slice TaskStatus to the external system's state.
  ...
  /**
   * Converts an external state + labels back to a Slice TaskStatus.
...
// ============================================================================
// Push: Slice Task → External Task
// ============================================================================

/**
 * Converts a Slice Task into an ExternalTaskInput for creating/updating
 ...
 * Slice assignees are ephemeral agents (e.g., el-xxxx) that don't
 * correspond to valid users on external platforms like GitHub.
 *
 * @param task - The Slice task to convert
...
export async function taskToExternalTask(
  ...
  return {
    ...
    state,
    labels,
    // Assignees are intentionally omitted — Slice assignees are ephemeral
...
// ============================================================================
// Pull: External Task → Slice Task Updates
// ============================================================================

/**
 * Converts an ExternalTask into a partial Task update object for applying
 * external changes to a local Slice task.
 *
 * Handles:
 * - Title mapping (1:1)
 ...
 *
 * @deprecated No longer used — Slice assignees are ephemeral agents
