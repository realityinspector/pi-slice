 ...
 * Maps between Slice ExternalTask/ExternalTaskInput and Linear's
...
import type { Timestamp, Priority } from '@slice/core';
import type {
  TaskSyncAdapter,
  ExternalTask,
  ExternalTaskInput,
...
import {
  ...
  linearPriorityToSlice,
  slicePriorityToLinear,
 ...
 *
 * Provides bidirectional sync between Slice tasks and Linear issues.
...
export class LinearTaskAdapter implements TaskSyncAdapter {
  ...
  private linearIssueToExternalTask(issue: LinearIssue, project: string): ExternalTask {
    ...
    return {
      ...
      labels,
      assignees,
      // Convert Linear native priority (0-4) to Slice priority (1-5)
      priority: linearPriorityToSlice(issue.priority),
  ...
  private async buildCreateInput(
    ...
    // Map priority: convert Slice priority (1-5) to Linear native priority (0-4).
    // If no priority is provided, default to Linear 0 (No priority).
    if (issue.priority !== undefined) {
      input.priority = slicePriorityToLinear(issue.priority as Priority);
    } else {
  ...
  private async buildUpdateInput(
    ...
    if (updates.priority !== undefined) {
      input.priority = slicePriorityToLinear(updates.priority as Priority);
    }
