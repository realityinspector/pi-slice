 ...
 *
 * Defines how Slice task fields map to Linear issue fields.
 *
 ...
 *
 * Priority mapping (Linear 0-4 ↔ Slice 1-5):
 *   Linear 1 (Urgent)      ↔ Slice 1 (critical)
 *   Linear 2 (High)        ↔ Slice 2 (high)
 *   Linear 3 (Medium)      ↔ Slice 3 (medium)
 *   Linear 4 (Low)         ↔ Slice 4 (low)
 *   Linear 0 (No priority) ↔ Slice 5 (minimal)
 *
...
import type { TaskStatus, Priority } from '@slice/core';
import type { TaskFieldMapConfig } from '@slice/core';
...
/**
 * Maps Slice TaskStatus values to label strings.
...
/**
 * Maps Slice priority (1-5) to Linear priority (0-4).
 *
 * Slice 1 (critical) → Linear 1 (Urgent)
 * Slice 2 (high)     → Linear 2 (High)
 * Slice 3 (medium)   → Linear 3 (Medium)
 * Slice 4 (low)      → Linear 4 (Low)
 * Slice 5 (minimal)  → Linear 0 (No priority)
 */
export function slicePriorityToLinear(priority: Priority): number {
  switch (priority) {
    case 1:
...
/**
 * Maps Linear priority (0-4) to Slice priority (1-5).
 *
 * Linear 1 (Urgent)      → Slice 1 (critical)
 * Linear 2 (High)        → Slice 2 (high)
 * Linear 3 (Medium)      → Slice 3 (medium)
 ...
 * Linear 0 (No priority) → Slice 5 (minimal)
 */
export function linearPriorityToSlice(priority: number): Priority {
  switch (priority) {
    case 1:
...
/**
 * Maps a Linear workflow state type to a Slice TaskStatus.
 *
 * Pull direction:
 *   triage    → backlog
 *   backlog   → backlog
...
/**
 * Maps a Slice TaskStatus to a Linear workflow state type.
 *
 * Push direction:
 *   open        → unstarted
 *   in_progress → started
...
/**
 * Returns true if the given Slice status should add a "blocked" label
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        direction: 'bidirectional',
        toExternal: 'slicePriorityToLinear',
        toLocal: 'linearPriorityToSlice',
      },
      {
        localField: 'tags',
 ...
 * lossless round-tripping. This ensures the exact Slice priority value
...
export function createLinearSyncFieldMapConfig(): TaskSyncFieldMapConfig {
  return {
    ...
    // labels alongside the native priority field, so the exact Slice
