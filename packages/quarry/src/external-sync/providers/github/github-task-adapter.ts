 ...
 *
 * Project format: 'owner/repo' (e.g., 'realityinspector/pi-slice')
...
import type { Timestamp } from '@slice/core';
...
function parseProject(project: string): [string, string] {
  ...
  if (parts.length !== 2 || !parts[0] || !parts[1]) {
    throw new Error(
      `Invalid GitHub project format: '${project}'. Expected 'owner/repo' (e.g., 'realityinspector/pi-slice').`
    );
  }
 ...
 *
 * Maps between Slice's normalized ExternalTask format and GitHub's
...
export class GitHubTaskAdapter implements TaskSyncAdapter {
   ...
   *
   * Note: Assignees are intentionally NOT set on created issues. Slice
  ...
  async createIssue(project: string, issue: ExternalTaskInput): Promise<ExternalTask> {
    ...
    const created = await this.client.createIssue(owner, repo, {
      ...
      // Assignees intentionally omitted — Slice agents are not GitHub users
    });
   ...
   * to map between Slice task fields and GitHub issue fields.
   *
