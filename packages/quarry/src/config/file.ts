...
import { ValidationError, ErrorCode } from '@slice/core';
...
/** Default .slice directory name */
export const SLICE_DIR = '.slice';
...
/**
 * Finds the nearest .slice directory by walking up from the given directory.
 *
 ...
 * @returns Path to .slice directory, or undefined if not found
 */
export function findSliceDir(startDir: string): string | undefined {
  ...
  // workspace's .slice directory where the SQLite database lives
  const envRoot = process.env.STONEFORGE_ROOT;
  if (envRoot) {
    const slicePath = path.join(envRoot, SLICE_DIR);
  ...
  while (currentDir !== root) {
    const slicePath = path.join(currentDir, SLICE_DIR);
    if (fs.existsSync(slicePath) && fs.statSync(slicePath).isDirectory()) {
      return slicePath;
    }
  ...
  // Check root
  const rootSlicePath = path.join(root, SLICE_DIR);
  if (fs.existsSync(rootSlicePath) && fs.statSync(rootSlicePath).isDirectory()) {
    return rootSlicePath;
  }
...
export function getGlobalConfigDir(): string {
  return path.join(os.homedir(), SLICE_DIR);
}
...
export function discoverConfigFile(
  ...
  if (overridePath) {
    ...
    return {
      path: resolvedPath,
      exists,
      sliceDir: exists ? path.dirname(resolvedPath) : undefined,
    };
  }

  // Try to find .slice directory
  const sliceDir = findSliceDir(startDir);
  if (sliceDir) {
    const configPath = path.join(sliceDir, CONFIG_FILE_NAME);
    return {
      path: configPath,
  ...
  return {
    ...
    sliceDir: fs.existsSync(getGlobalConfigDir()) ? getGlobalConfigDir() : undefined,
  };
}
...
export function writeConfigFile(
  ...
  const content = `# Slice Configuration\n\n${serializeConfigToYaml(config)}`;
